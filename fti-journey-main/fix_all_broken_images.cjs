...
const brokenCyprusUrl1 = 'https://images.unsplash.com/photo-1558981285-6f0c68a8df6d?w=800&h=500&fit=crop';
const brokenCyprusUrl2 = 'https://images.unsplash.com/photo-1558981285-6f0c68a8df6d';
const workingCyprusUrl = 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=1200';

const wikimediaReplacements = {
  ...
  'https://upload.wikimedia.org/wikipedia/commons/thumb/3/30/Paphos_harbour_castle.jpg/1200px-Paphos_harbour_castle.jpg': workingCyprusUrl,
...
function processFile(filePath) {
  ...
  // Replace exact broken cyprus url strings
  if (content.includes(brokenCyprusUrl1)) {
    content = content.split(brokenCyprusUrl1).join(workingCyprusUrl);
    changed = true;
  }
  
  // Replace base broken cyprus url with our reliable URL in any variation
  if (content.includes(brokenCyprusUrl2)) {
      // RegEx to catch the base URL and any query params after it inside the quotes
      const regex = new RegExp(brokenCyprusUrl2.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '[^"\']*', 'g');
      content = content.replace(regex, workingCyprusUrl);
      changed = true;
  }

  // Replace all wikimedia links
  for (const [oldUrl, newUrl] of Object.entries(wikimediaReplacements)) {
...
searchDir('src');
console.log('All broken URLs replaced globally!');
