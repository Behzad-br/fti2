...
const PTE = () => {
  ...
  const rawPteImages = cmsData.pteSuccessImages?.length
    ? cmsData.pteSuccessImages
    : (isCmsReady ? ['/success-stories/pte/pte-1.jpg', '/success-stories/pte/pte-2.jpg'] : []);
  const pteStoryImages = [
    ...rawPteImages.filter((src) => src.startsWith('data:')),
    ...rawPteImages.filter((src) => !src.startsWith('data:')),
  ];
